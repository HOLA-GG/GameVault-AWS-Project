...
## 2026-07-05 - Programmatic Accessibility Announcements for Async Actions
**Learning:** For dynamic, non-visual components like asynchronously submitted user ratings and client-side media file selections, screen-reader users remain unaware of status updates unless we programmatically announce them. Centralizing dynamic updates via a global screen-reader utility (e.g., `window.announceToScreenReader`) and wrapping calls defensively with optional chaining `?.` provides immediate contextual feedback while keeping the JavaScript runtime robust if the helper hasn't loaded.
**Action:** Always pair visual state changes (like showing a file preview or dynamic feedback messages) with a fallback-safe `window.announceToScreenReader?.()` announcement detailing the action's outcome.
